...
- Dial mode:
  ...
  - `Custom`: enables the current HID/raw input capture path for diagnostics, but does not apply ChatMix changes yet.
...
Nova 7 dial support is enabled by default through `Dial mode: Nova 7`. The app looks for SteelSeries `VID 1038` / `PID 2202` HID devices and prioritizes the vendor usage page used by the ChatMix report. Summary diagnostics are written to `%AppData%\SonarSlideVB\app.log`.
...
`Dial mode: Custom` is currently capture-only. It is useful for collecting HID/raw input diagnostics in `app.log`, but custom device mapping and ChatMix application are not supported yet. Keyboard raw input values are not logged. When `app.log` grows beyond 1 MB, the previous file is rotated to `app.log.1`.
